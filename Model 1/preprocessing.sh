...
if [[ ${SRC_FILE} == "mt" || ${TGT_FILE} == "mt" ]]; then
    ...
    cp -rf original_data/${SRC_FILE}_${TGT_FILE}/*.${SRC_FILE} data/${SRC_FILE}_${TGT_FILE}/
    cp -rf original_data/${SRC_FILE}_${TGT_FILE}/*.${TGT_FILE} data/${SRC_FILE}_${TGT_FILE}/
    mkdir -p ${MODELS} ${CHECKPOINTS} ${LOGS} ${DATA_OUT} ${TRANSLATIONS}

elif [[ ${SRC_FILE} == "ic" || ${TGT_FILE} == "ic" ]]; then
    #rm -rf results/
    echo "working on: "
    echo original_data/${SRC_FILE}_${TGT_FILE}
    rm -rf data/${SRC_FILE}_${TGT_FILE}/
    #
    mkdir -p data/${SRC_FILE}_${TGT_FILE}/
    cp -rf original_data/${SRC_FILE}_${TGT_FILE}/*.${SRC_FILE} data/${SRC_FILE}_${TGT_FILE}/
...
for file in ${DATA_DIR}/*.${SRC_FILE}; do
  ...
  echo "REMOVING: "
  echo ${DATA_DIR}/$(basename $file).tok
  ls
