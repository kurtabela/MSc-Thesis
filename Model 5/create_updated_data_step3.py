...
import json
with open("results_mt.txt", "r", encoding="utf-8") as results_mt_file:
    results_mt = json.load(results_mt_file)
...
class Translator(tf.Module):
    def __init__(self, tokenizers, transformer, SRC_LANG, TGT_LANG, tokenize_pairs, beam_search):
    ...
    def __call__(self, sentences, beam_width=3):
        ...
        if SRC_LANG == "en" and TGT_LANG == "mt":
            ...
            vocab_length = tokenizers.mt.get_vocab_size()
        else:
        ...
        for i, score in enumerate(scores):
            ...
            if SRC_LANG == "en" and TGT_LANG == "mt":
                text = tokenizers.mt.detokenize(output)
            #             tokens = tokenizers.mt.lookup(output)[0]
            else:
                text = tokenizers.en.detokenize(output)
...
def beam_search(symbols_to_logits_fn,
    ...
    def grow_topk(i, alive_seq, alive_log_probs, states):
        ...
        if states:
            ...
            flat_logits = symbols_to_logits_fn.predict(ds, verbose=0)
...
model_name = '../test_dataset/ted_hrlr_translate_mt_en_converter'
# model_name = '/netscratch/abela/transformerbaseline/test_dataset/ted_hrlr_translate_mt_en_converter'
...
transformer_test_mt_en = transformer(
    input_vocab_size=tokenizers.mt.get_vocab_size().numpy(),
...
transformer_test_mt_en.load_weights('../test_dataset2/translator_for_confidence_mt_en')
#transformer_test_mt_en.load_weights('/netscratch/abela/transformerbaseline/test_dataset/translator_for_confidence_mt_en')
translator = Translator(tokenizers, transformer_test_mt_en, "mt", "en", tokenize_pairs, beam_search)
...
#data_in = data_in[:i+too_big_predictions+100]
orig_data_in = copy.deepcopy(data_in)
data_in = list(map((lambda x: x["multi_sense_sentence"]), data_in))
mt_list = tokenizers.mt.tokenize(data_in)
...
# with open('new_data.mt', "w+", encoding="utf-8") as mt, open('new_data.en', "w+", encoding="utf-8") as en:
with open('todelete.mt', "w+", encoding="utf-8") as mt, open('todelete.en', "w+", encoding="utf-8") as en:
    noMaskReturned = 0
    print(len(orig_data_in))
    for i, result in enumerate(orig_data_in):
        print(i)
        source_sentence = result["multi_sense_sentence"].replace(result["multi_sense_word"], "<MASK>")
        to_tok = tokenizers.mt.tokenize([source_sentence])
        if tf.shape(to_tok.to_tensor())[1] >= MAX_TOKEN_LENGTH:
            print("too big...")
            too_big_indxs.append(i)
        else:
            print(source_sentence)
            target_sentence = translator([tf.constant(source_sentence)])[0].numpy()[0].decode('utf-8')
            print(target_sentence)

            if "<MASK>" in target_sentence:
                target_sentence = target_sentence.replace("<MASK>", result["target_word"])
            else:
                noMaskReturned += 1             
            en.write(target_sentence + "\n")
            mt.write(result["multi_sense_sentence"] + "\n")
        
    print("MT -> EN")
    print("Total translations without <MASK> in the target: " + str(noMaskReturned/len(results_mt)))



# %%
import os
os.system(" cp ../../multisense/train.* ../datasetwithoutmasks/data")

os.system("cp /netscratch/abela/dfkibaseline/mono_mt_results/translations/en_mt/en_mt.src ../")
os.system("cp /netscratch/abela/dfkibaseline/mono_mt_results/translations/en_mt/en_mt.hyp ../")



os.system("../test_dataset/data/detokenizer.perl -l en < new_data.en > new_data.en.detok")



# %%

os.system("wc -l ../datasetwithoutmasks/data/train.en")
os.system("wc -l ../datasetwithoutmasks/data/train.mt")

# %%

os.system(" python ../tokenisemt.py --src new_data.mt --tgt results_mt.txt.detokenized --decode")


with open("results_mt.txt.detokenized", "r") as outfile:
    
    print(outfile.readlines()[0])

# %%
os.system("wc -l ../en_mt.src")
os.system("wc -l ../en_mt.hyp")
with open("../en_mt.src", "r", encoding="utf-8") as f:
    data = f.read()
    print("Read 1")

with open("../en_mt.hyp", "r", encoding="utf-8") as f:
    data = f.read()
    print("Read 1")

# %%
with open("new_data.en.detok", "r") as outfile:
...
input_file_src = "../en_mt.src"
input_file_tgt = "../en_mt.hyp"
multi_sense_file = "new_data.mt"
multi_sense_res_file = "new_data.en"

with open(input_file_tgt, "r", encoding="utf-8") as f:
    input_file_tgt_lines = f.readlines()
    
    
with open(input_file_src, "r", encoding="utf-8") as f:
    monolingual_inp = f.readlines()
    for i, inp in enumerate(monolingual_inp):
        monolingual_inp[i] = inp.replace("\n", "")
        
        
print(len(input_file_tgt_lines))
print(len(monolingual_inp))
with open(multi_sense_res_file, "r", encoding="utf-8") as f:
    multi_sense_res = f.readlines()
    for i, inp in enumerate(multi_sense_res):
        multi_sense_res[i] = inp.replace("\n", "")

with open(multi_sense_file, "r", encoding="utf-8") as f:
    multi_sense_file_inp = f.readlines()
    for i, inp in enumerate(multi_sense_file_inp):
        multi_sense_file_inp[i] = inp.replace("\n", "")

print(len(multi_sense_file_inp))
print(len(multi_sense_res))


# %%

    
with open("new_data.en", "r") as outfile:
    
    print(outfile.readlines()[0])

# %%
print(multi_sense_file_inp[0])

# %%
indexesToRemove = []
indexesToRemoveFromResultsFile = []
toAddInstead = []

with open("results_mt.txt.detokenized", "r", encoding="utf-8") as f:
    multi_sense_file_inp = f.readlines()
    for i, inp in enumerate(multi_sense_file_inp):
        multi_sense_file_inp[i] = inp.replace("\n", "")
print(monolingual_inp[0])
print(multi_sense_file_inp[0])
for i, inp in enumerate(monolingual_inp):
    if inp in multi_sense_file_inp:
        indexesToRemove.append(i)
        multi_sense_idx = multi_sense_file_inp.index(inp)
        indexesToRemoveFromResultsFile.append(multi_sense_idx)
        toAddInstead.append(multi_sense_idx)

print(indexesToRemove)
print(indexesToRemoveFromResultsFile)
print(toAddInstead)
...
with open("new_data.en.detok", "r", encoding="utf-8") as f:
    multi_sense_res = f.readlines()
    for i, inp in enumerate(multi_sense_res):
        multi_sense_res[i] = inp.replace("\n", "")
print(multi_sense_res[0])
print(len(multi_sense_res))
...
# %%
print(len(toAddInstead))
print(monolingual_inp[0])
print(input_file_tgt_lines[0])

# %%

with open(input_file_src, "w+", encoding="utf-8") as src, open(input_file_tgt, "w+", encoding="utf-8") as tgt:
    print(len(input_file_tgt_lines))
    print(len(monolingual_inp))
    print("writing to: ")
    print(input_file_src)
    print(input_file_tgt)
    for i, line in enumerate(monolingual_inp):
        if i % 1000000 == 0:
            print(i)
        if i not in indexesToRemove:
            src.write(line + "\n")
            
    for i, line in enumerate(input_file_tgt_lines):
        if i % 1000000 == 0:
            print(i)
        if i not in indexesToRemove:
            tgt.write(line)
    print("done writing to tgt")
    
     
    print("done writing to src")       
            
#     Add the new data
    for i in toAddInstead:
        src.write(multi_sense_file_inp[i]  + "\n" )
        tgt.write(multi_sense_res[i] + "\n")
...
with open("../datasetwithoutmasks/data/train.mt", "r", encoding="utf-8") as f:
    data = f.read()
    print("Read 1")

with open("../datasetwithoutmasks/data/train.en", "r", encoding="utf-8") as f:
    data = f.read()
    print("Read 2")
...
os.system("cat ../en_mt.src >> ../datasetwithoutmasks/data/train.mt")
os.system("cat ../en_mt.hyp >> ../datasetwithoutmasks/data/train.en")
os.system("wc -l ../datasetwithoutmasks/data/train.en")
os.system("wc -l ../datasetwithoutmasks/data/train.mt")
...
# os.system(" iconv -f UTF-8 ../datasetwithoutmasks/data/train.mt -t UTF-8 -o /dev/null; echo $"")

# %%
# def run(task_id, i, too_big_predictions, lengthOfExamples):
#     batches = []
#     print("in rin")
#     with open(os.path.dirname(os.path.realpath("__file__")) + '/data/predictions' + str(
#             task_id) + '.' + TGT_LANG, "a+",
#               encoding="utf-8") as w:
#         with open(os.path.dirname(os.path.realpath("__file__")) + '/data/tooBigPredictions' + str(
#                 task_id) + '.' + TGT_LANG, "a+", encoding="utf-8") as wTooBig:
#             print(lengthOfExamples)
#             while (i + too_big_predictions + batch_size) <= lengthOfExamples - 1:
#                 if i % 100 == 0:
#                     print(i)
#                 to_predict = []
#                 if SRC_LANG == "en":
#                     for curr_index, en in enumerate(
#                             en_list[i + too_big_predictions: i + too_big_predictions + batch_size]):
#                         if tf.shape(en_list[i + too_big_predictions + curr_index])[0] >= MAX_TOKEN_LENGTH:
#                             wTooBig.write(str(curr_index))
#                             too_big_predictions += 1
#                         else:
#                             try:
#                                 print(data_in[i + too_big_predictions + curr_index])
#                                 print(data_in[i + too_big_predictions + curr_index].replace(orig_data_in[i + too_big_predictions + curr_index]["multi_sense_word"], "<MASK>"))
#                                 to_predict.append(tf.constant(data_in[i + too_big_predictions + curr_index].replace(orig_data_in[i + too_big_predictions + curr_index]["multi_sense_word"], "<MASK>")))
#                             except:
#                                 pass

#                 if SRC_LANG == "mt":
#                     for curr_index, mt in enumerate(
#                             mt_list[i + too_big_predictions: i + too_big_predictions + batch_size]):
#                         if tf.shape(mt_list[i + too_big_predictions + curr_index])[0] >= MAX_TOKEN_LENGTH:
#                             wTooBig.write(str(curr_index))
#                             too_big_predictions += 1
#                         else:
#                             try:
...
#                             except:
#                                 print("Issue?")
#                                 print(i + too_big_predictions + curr_index)
#                                 print(len(data_in))
#                                 print(data_in[i + too_big_predictions + curr_index])
#                                 print("PRINTED ALL")
#                                 pass
#                 if to_predict:
#                     # batches.append(to_predict)
#                     print("PRINTING TO " + os.path.dirname(
#                         os.path.realpath("__file__")) + '/data/predictions' + str(
#                         task_id) + '.' + TGT_LANG)
#                     output = translator(to_predict)
#                     # print("predicted")
...
#     parser.add_argument("--task_id", help="Task ID", type=int,  required=False, default="1")
#     args = parser.parse_args(args=[])
#     #args = parser.parse_args()
#     args.task_id = str(args.task_id).zfill(2)
#     print(args.task_id)

#     model_name = "../test_dataset/ted_hrlr_translate_mt_en_converter"

#     tokenizers = tf.saved_model.load(model_name)
#     vocab_length_en = tokenizers.en.get_vocab_size()
#     vocab_length_mt = tokenizers.mt.get_vocab_size()
...
#     too_big_predictions = []
#     toWrite = []
#     with open("results_mt" + str(args.task_id) + '.txt', "r", encoding="utf-8") as results_mt_file:
#         results_mt = json.load(results_mt_file)
...
#     if os.path.exists(
#             os.path.dirname(os.path.realpath("__file__")) + '/baseline_preds' + str(args.task_id) + '.' + TGT_LANG) == False:
#         open(os.path.dirname(os.path.realpath("__file__")) + '/baseline_preds' + str(args.task_id) + '.' + TGT_LANG, "w",
#           encoding="utf-16").close()

#     with open(os.path.dirname(os.path.realpath("__file__")) + '/baseline_preds' + str(args.task_id) + '.' + TGT_LANG, "r+",
#           encoding="utf-16") as fp:
#         i = len(fp.readlines())
#     print('Total lines so far:', i)

#     if os.path.exists(os.path.dirname(
#             os.path.realpath("__file__")) + '/tooBigPredictions' + str(args.task_id) + '.' + TGT_LANG) == False:
#         open(os.path.dirname(os.path.realpath("__file__")) + '/tooBigPredictions' + str(args.task_id) + '.' + TGT_LANG,
#              "w").close()

#     with open(os.path.dirname(os.path.realpath("__file__")) + '/tooBigPredictions' + str(args.task_id) + '.' + TGT_LANG, "r+",
#               encoding="utf-16") as fp:
...
#     data_in = list(map((lambda x: x["multi_sense_sentence"]), data_in))
#     if SRC_LANG == "en":
#         en_list = tokenizers.en.tokenize(data_in)

#     if SRC_LANG == "mt":
#         mt_list = tokenizers.mt.tokenize(data_in)
#     #data_in = data_in[:i+too_big_predictions+10000]
#     print(i)
#     lengthOfExamples = len(data_in)
#     print(lengthOfExamples)


#     run(args.task_id, i, too_big_predictions, lengthOfExamples)
#     print("DONE?")
# %%
...
# with open('data/train.mt', 'a+') as outfile:
#     with open('new_data.mt') as infile:
#         for line in infile:
#             outfile.write(line)
            
# with open('data/train.en', 'a+') as outfile:
#     with open('new_data.en') as infile:
#         for line in infile:
#             outfile.write(line)
            
# #os.system("tfds build --overwrite --data_dir " + os.getcwd())
