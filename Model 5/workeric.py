...
class Translator(tf.Module):
  ...
  def __call__(self, sentences, beam_width=3):
      ...
      for i, sentence in enumerate(sentences):
          ...
          if  self.SRC_LANG == "en" and  self.TGT_LANG == "ic":
...
#        print(self.translator([tf.constant("w")]))
    ...
    def filter_max_tokens(self, en, ic):
            en = self.tokenizers.en.tokenize(en)
            ic = self.tokenizers.ic.tokenize(ic)
            en = en.to_tensor()
            ic = ic.to_tensor()
            #
            #     print(ic)
    ...
    def run(self):
        ...
        if self.SRC_LANG == "en" and self.TGT_LANG == "ic":
            ...
            transformer_test = self.transformer(
                ...
                num_heads=NUM_HEADS,
                dropout=DROPOUT)
        transformer_test.load_weights('./ic_en_dataset/translator_for_confidence_' + self.SRC_LANG + "_" + self.TGT_LANG)
    ...
    def predict(self, text):
        with open(os.path.dirname(os.path.realpath("__file__")) + '/ic_en_dataset/data/predictions.' + self.TGT_LANG, "a+",
                  encoding="utf-8") as w:
            #             print(text)
            #print("predicting...")
