...
# %%
print("BEGINNING STEP 3 IC")

import json
with open("results_ic.txt", "r", encoding="utf-8") as results_ic_file:
    results_ic = json.load(results_ic_file)
...
class Translator(tf.Module):
    ...
    def __call__(self, sentences, beam_width=3):
        ...
        if SRC_LANG == "en" and TGT_LANG == "ic":
            ...
            vocab_length = tokenizers.ic.get_vocab_size()
        else:
        ...
        for i, score in enumerate(scores):
            ...
            if SRC_LANG == "en" and TGT_LANG == "ic":
                text = tokenizers.ic.detokenize(output)
...
transformer_test_ic_en.load_weights('../en_ic_dataset/translator_for_confidence_ic_en')
#transformer_test_ic_en.load_weights('/netscratch/abela/transformerbaseline/ic_en_dataset/translator_for_confidence_ic_en')
translator = Translator(tokenizers, transformer_test_ic_en, "ic", "en", tokenize_pairs, beam_search)
...
ic_list = tokenizers.ic.tokenize(data_in)
...
with open('new_data.ic', "w+", encoding="utf-8") as ic, open('new_data.en', "w+", encoding="utf-8") as en:
    noMaskReturned = 0
    ...
    for i, result in enumerate(orig_data_in):
        ...
        to_tok = tokenizers.ic.tokenize([source_sentence])
        if tf.shape(to_tok.to_tensor())[1] >= MAX_TOKEN_LENGTH:
            ...
            ic.write(result["multi_sense_sentence"] + "\n")
        
    print("ic -> EN")
    print("Total translations without <MASK> in the target: " + str(noMaskReturned/len(results_ic)))
...
os.system(" cp ../../multisense/train.* data")
os.system("cp /netscratch/abela/dfkibaseline/mono_ic_results/translations/en_ic/en_ic.src ../")
os.system("cp /netscratch/abela/dfkibaseline/mono_ic_results/translations/en_ic/en_ic.hyp ../")

os.system("../test_dataset/data/detokenizer.perl -l en < new_data.en > new_data.en.detok")

# %%

os.system(" python ../tokeniseic.py --src new_data.ic --tgt results_ic.txt.detokenized --decode")
...
input_file_src = "../en_ic.src"
input_file_tgt = "../en_ic.hyp"
multi_sense_file = "new_data.ic"
...
with open("results_ic.txt.detokenized", "r") as outfile:
...
with open("results_ic.txt.detokenized", "r", encoding="utf-8") as f:
    multi_sense_file_inp = f.readlines()
    for i, inp in enumerate(multi_sense_file_inp):
        multi_sense_file_inp[i] = inp.replace("\n", "")
...
print(len(multi_sense_res))
with open(input_file_src, "w+") as src, open(input_file_tgt, "w+") as tgt:
...
os.system("cat ../en_ic.src >> ../dataset_without_masks_ic/data/train.ic")
os.system("cat ../en_ic.hyp >> ../dataset_without_masks_ic/data/train.en")
os.system("wc -l ../dataset_without_masks_ic/data/train.ic")
os.system("wc -l ../dataset_without_masks_ic/data/train.en")
...
#                         else:
#                             try:
#                                 to_predict.append(tf.constant(data_in[i + too_big_predictions + curr_index]))
#                             except:
#                                 pass

#                 if SRC_LANG == "ic":
#                     for curr_index, ic in enumerate(
#                             ic_list[i + too_big_predictions: i + too_big_predictions + batch_size]):
#                         if tf.shape(ic_list[i + too_big_predictions + curr_index])[0] >= MAX_TOKEN_LENGTH:
#                             wTooBig.write(str(curr_index))
#                             too_big_predictions += 1
#                         else:
#                             try:
...
#     vocab_length_ic = tokenizers.ic.get_vocab_size()
...
#     too_big_predictions = []
#     toWrite = []
#     with open("results_ic" + str(args.task_id) + '.txt', "r", encoding="utf-8") as results_ic_file:
#         results_ic = json.load(results_ic_file)
...
# with open('data/train.ic', 'a+') as outfile:
#     with open('new_data.ic') as infile:
#         for line in infile:
#             outfile.write(line)
