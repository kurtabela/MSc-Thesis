...
class Translator(tf.Module):
  ...
  def __call__(self, sentences, beam_width=3):
      ...
      for i, sentence in enumerate(sentences):
          ...
          if  self.SRC_LANG == "en" and  self.TGT_LANG == "mt":
              (sentence_tok, __), _ = self.tokenize_pairs(sentence, [''])
          else:
              (sentence_tok, __), _ = self.tokenize_pairs([''], sentence)
...
#        print("predicting in init...")
#
#        print(self.translator([tf.constant("w")]))
    ...
    def _merge_beam_dim(self,tensor):
    ...
    def _unmerge_beam_dim(self,tensor, batch_size, beam_size):
    ...
    def _expand_to_beam_size(self,tensor, beam_size):
    ...
    def get_state_shape_invariants(self,tensor):
    ...
    def compute_batch_indices(self,batch_size, beam_size):
    ...
    def fast_tpu_gather(self,params, indices, name=None):
    ...
    def _create_make_unique(self,inputs):
    ...
    def _create_topk_unique(self,inputs, k):
    ...
    def top_k_with_unique(self, inputs, k):
        ...
        """
        unique_inputs = self._create_make_unique(tf1.cast(inputs, tf1.float32))
        top_values, indices = self._create_topk_unique(unique_inputs, k)
        top_values = tf1.cast(top_values, inputs.dtype)
        return top_values, indices
    
    
    def compute_topk_scores_and_seq(self, sequences,
                                    scores,
                                    scores_to_gather,
                                    flags,
                                    beam_size,
        ...
        if not use_tpu:
            ...
            # batch the beam item is in. This will create the i of the i,j coordinate
            # needed for the gather
            batch_pos = self.compute_batch_indices(batch_size, beam_size)
            ...
            if use_top_k_with_unique:
                _, topk_indexes = self.top_k_with_unique(scores, k=beam_size)
            else:
                _, topk_indexes = tf1.nn.top_k(scores, k=beam_size)
            # Gather up the highest scoring sequences.  For each operation added, give
            # it a concrete name to simplify observing these operations with tf1dbg.
            # Clients can capture these tensors by watching these node names.
            topk_seq = self.fast_tpu_gather(sequences, topk_indexes, prefix + "_topk_seq")
            topk_flags = self.fast_tpu_gather(flags, topk_indexes, prefix + "_topk_flags")
            topk_gathered_scores = self.fast_tpu_gather(scores_to_gather, topk_indexes,
                                                   prefix + "_topk_scores")
            if states_to_gather:
                topk_gathered_states = nest.map_structure(
                    # pylint: disable=g-long-lambda
                    lambda state: self.fast_tpu_gather(state, topk_indexes,
                                                  prefix + "_topk_states"),
                    states_to_gather)
            else:
                topk_gathered_states = states_to_gather
    ...
    def beam_search(self, symbols_to_logits_fn,
                    initial_ids,
                    beam_size,
                    decode_length,
                    vocab_size,
        ...
        alive_seq = self._expand_to_beam_size(initial_ids, beam_size)
        ...
        def grow_finished(finished_seq, finished_scores, finished_flags, curr_seq,
            ...
            return self.compute_topk_scores_and_seq(
                curr_finished_seq,
        ...
        def grow_alive(curr_seq, curr_scores, curr_log_probs, curr_finished, states):
            ...
            return self.compute_topk_scores_and_seq(curr_seq, curr_scores, curr_log_probs,
        ...
        def grow_topk(i, alive_seq, alive_log_probs, states):
            ...
            if not use_tpu:
                ...
                # items that survived.
                batch_pos = self.compute_batch_indices(batch_size, beam_size * 2)
                ...
                # Gather up the most probable 2*beams both for the ids and
                # finished_in_alive bools
                topk_seq = self.fast_tpu_gather(alive_seq, topk_beam_index)
    
                if states:
                    states = nest.map_structure(
                        lambda state: self.fast_tpu_gather(state, topk_beam_index), states)
        ...
        state_struc = nest.map_structure(self.get_state_shape_invariants, states)
    ...
    def transformer(self, input_vocab_size,
                    target_vocab_size,
                    num_layers,
                    units,
                    d_model,
    ...
    def filter_max_tokens(self, en, mt):
            en = self.tokenizers.en.tokenize(en)
            mt = self.tokenizers.mt.tokenize(mt)
            en = en.to_tensor()
            mt = mt.to_tensor()
            #
            #     print(mt)
    ...
    def run(self):
        ...
        print(tf.config.list_physical_devices('GPU'))
        print("SER DEVIDE ORDER")
        print("initially, CUDA_VISIBLE_DEVICES is: ")
        print(os.environ['CUDA_VISIBLE_DEVICES'])
        os.environ["CUDA_VISIBLE_DEVICES"] = str(self._gpuid)
        print(tf.config.list_physical_devices('GPU'))
        print("SET CUDA_VISIBLE_DEVICES TO: ")
        print(os.environ['CUDA_VISIBLE_DEVICES'])
        
        gpus = tf.config.experimental.list_physical_devices('GPU')
        print("getting visible gpus...")
        print(gpus)
        for gpu in gpus:
          tf.config.experimental.set_memory_growth(gpu, True)        
        
        print("using tf method now...")
        try:
          tf.config.experimental.set_visible_devices(gpus[self._gpuid], 'GPU')
        ...
        DROPOUT = 0.4
        if self.SRC_LANG == "en" and self.TGT_LANG == "mt":
            transformer_test = self.transformer(
            ...
            transformer_test = self.transformer(
                ...
                num_heads=NUM_HEADS,
                dropout=DROPOUT)
        transformer_test.load_weights('./test_dataset/translator_for_confidence_' + self.SRC_LANG + "_" + self.TGT_LANG)
        
        translator = Translator(tokenizers, transformer_test, self.SRC_LANG, self.TGT_LANG, self.tokenize_pairs, self.beam_search)  
        ...
        while True:
            ...
            self.predict(xfile)
            print('worker', self._gpuid, " predicted ")

        print('vggnet done ', self._gpuid)

    def predict(self, text):
        with open(os.path.dirname(os.path.realpath("__file__")) + '/test_dataset/data/predictions.' + self.TGT_LANG, "a+",
