...
total_lines=$(wc -l < /netscratch/abela/multi-sense-words/test_dataset/data/test.en )
((lines_per_file = (total_lines ) / (${SLURM_NTASKS}-1)))

split --lines=${lines_per_file} --numeric-suffixes=1 --suffix-length=2 --additional-suffix=".en" --verbose /netscratch/abela/multi-sense-words/test_dataset/data/test.en /netscratch/abela/multi-sense-words/test_dataset/data/test



total_lines=$(wc -l < /netscratch/abela/multi-sense-words/test_dataset/data/test.mt )
((lines_per_file = (total_lines) / (${SLURM_NTASKS}-1)))

split --lines=${lines_per_file} --numeric-suffixes=1 --suffix-length=2 --additional-suffix=".mt" --verbose /netscratch/abela/multi-sense-words/test_dataset/data/test.mt /netscratch/abela/multi-sense-words/test_dataset/data/test
