...
if SRC_LANG == "en":
    ...
    examples, metadata = tfds.load(name="test_dataset:0.0.2", with_info=True,
                               as_supervised=True, data_dir=os.getcwd() , download=True)
else:
#     import test_dataset2
    os.chdir("test_dataset2")
#    os.system("tfds build --data_dir " + os.getcwd())
    examples, metadata = tfds.load(name="test_dataset2:0.0.2", with_info=True,
                               as_supervised=True, data_dir=os.getcwd() , download=True)
...
# %%
print("\n\n\n Should say Hawn illum!\n\n")
tokens = tokenizers.mt.tokenize(['Hawn illum!'])
print(tokens.numpy())
text_tokens = tokenizers.mt.detokenize(tokens)
print(text_tokens)
...
checkpoint_path = "./checkpoints/train-baseline/" +  SRC_LANG + "_" + TGT_LANG + "/cp.ckpt"
...
transformer_test.fit(train_batches, epochs=EPOCHS, batch_size=BATCH_SIZE, validation_data=val_batches)
...
#         print(id)
      ...
      options = tf.data.Options()
      options.experimental_distribute.auto_shard_policy = tf.data.experimental.AutoShardPolicy.DATA
      ds = tf.data.Dataset.from_tensor_slices((batch_inputs)).map(lambda x: {'inputs': x[0], "dec_inputs": x[1]}).with_options(options)
...
#     )
      ...
      steps = len(batch_inputs)
      flat_logits = symbols_to_logits_fn.predict(dist_dataset, steps=steps)
      new_array = []
...
#    
#    
#with open(os.path.dirname(os.path.realpath("__file__")) + '/data/predictions.' + TGT_LANG, "r+", encoding="utf-16") as fp:
#    i = len(fp.readlines())
...
#    
#with open(os.path.dirname(os.path.realpath("__file__")) + '/data/tooBigPredictions.' + TGT_LANG, "r+", encoding="utf-16") as fp:
...
#with open(os.path.dirname(os.path.realpath("__file__")) + '/data/predictions.' + TGT_LANG, "a+", encoding="utf-16") as w:
#    with open(os.path.dirname(os.path.realpath("__file__")) + '/data/tooBigPredictions.' + TGT_LANG, "a+", encoding="utf-16") as wTooBig:
...
#                if SRC_LANG == "mt":
#                    mt = tokenizers.mt.tokenize([data_in[curr_index]])
#                    mt = mt.to_tensor()
#                    if tf.shape(mt)[1] >= MAX_TOKEN_LENGTH:
#                        print("too big")
#                        wTooBig.write(str(curr_index))
#                        too_big_predictions += 1
#                    else:
