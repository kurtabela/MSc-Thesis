...
def beam_search(symbols_to_logits_fn,
 ...
 #           flat_logits = symbols_to_logits_fn.predict(ds, use_multiprocessing=True, workers=40)
            new_array = []
...
if SRC_LANG == "en" and TGT_LANG == "mt":
    ...
    transformer_test = transformer(
        ...
        num_heads=NUM_HEADS,
        dropout=DROPOUT)
transformer_test.load_weights('./datasetwithoutmasks/translator_for_confidence_' + SRC_LANG + "_" + TGT_LANG)
...
#         with open(os.path.dirname(os.path.realpath("__file__")) + '/datasetwithoutmasks/data/predictions' + str(nValue) + '.' + TGT_LANG, "a+",
#               encoding="utf-8") as w:
#
#
#
#             for xfile in xfilelst:
#               print("PRINTING TO " + os.path.dirname(os.path.realpath("__file__")) + '/datasetwithoutmasks/data/predictions' + str(nValue) + '.' + TGT_LANG)
...
#
#
# #        Parallel(n_jobs=int(nValue), backend="threading", require='sharedmem')(delayed(func2)(xfile, nValue) )
#
#
...
def run(task_id, i, too_big_predictions, lengthOfExamples, test_tgt_lang):
    ...
    with open(os.path.dirname(os.path.realpath("__file__")) + '/datasetwithoutmasks/data/predictions' + str(task_id) + '.' + TGT_LANG, "a+",
              encoding="utf-8") as w:
        with open(os.path.dirname(os.path.realpath("__file__")) + '/datasetwithoutmasks/data/actualTest' + str(task_id) + '.' + TGT_LANG, "a+", encoding="utf-8") as wTooBig:
            print("doing while....")
            ...
            while (i+too_big_predictions + batch_size) <= lengthOfExamples-1:
                ...
                if to_predict:
                   ...
                   print("PRINTING TO " + os.path.dirname(os.path.realpath("__file__")) + '/datasetwithoutmasks/data/predictions' + str(task_id) + '.' + TGT_LANG)
...
if __name__ == '__main__':
    ...
    toWrite = []
    data_in = []
    with open(os.path.dirname(os.path.realpath("__file__")) + '/datasetwithoutmasks/data/test' + str(args.task_id) + '.' + SRC_LANG, "r+",
              encoding="utf-8") as f:
        data_in = f.read().split('\n')
    with open(os.path.dirname(os.path.realpath("__file__")) + '/datasetwithoutmasks/data/test' + str(args.task_id) + '.' + TGT_LANG, "r+",
              encoding="utf-8") as f:
    ...
    open(os.path.dirname(os.path.realpath("__file__")) + '/datasetwithoutmasks/data/predictions' + args.task_id + '.' + TGT_LANG, "w+").close()
    open(os.path.dirname(os.path.realpath("__file__")) + '/datasetwithoutmasks/data/actualTest' + str(
            args.task_id) + '.' + TGT_LANG, "w+").close()

    with open(os.path.dirname(os.path.realpath("__file__")) + '/datasetwithoutmasks/data/predictions'+ args.task_id + '.' + TGT_LANG, "r+",
              encoding="utf-8") as fp:
        i = len(fp.readlines())
    print('Total lines so far:', i)

    if os.path.exists(os.path.dirname(
            os.path.realpath("__file__")) + '/datasetwithoutmasks/data/actualTest' + args.task_id + '.' + TGT_LANG) == False:
        open(os.path.dirname(os.path.realpath("__file__")) + '/datasetwithoutmasks/data/actualTest' + args.task_id + '.' + TGT_LANG,
             "w+").close()

    with open(os.path.dirname(os.path.realpath("__file__")) + '/datasetwithoutmasks/data/actualTest' + args.task_id + '.' + TGT_LANG, "r+",
              encoding="utf-8") as fp:
