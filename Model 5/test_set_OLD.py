...
#            print(e)
    ...
    with open(os.path.dirname(os.path.realpath("__file__")) + '/test_dataset/data/predictions.' + TGT_LANG, "a+",
...
class Scheduler:
    def __init__(self, gpuids, cpuids):
        print("Scheduker init")
        self._queue = Queue()
        self._gpuids = gpuids
        self._cpuids = cpuids
        print("Scheduker init done")
...
#        self._workers = list()
#        for ind in range(3):
#            self._workers.append(Vgg16Worker(self._gpuids[0],self._cpuids, self._queue))

    def start(self, xfilelst):
    
    

        number_of_tasks = int(os.environ['SLURM_NTASKS'])
        # number of cores you have allocated for your slurm task:
        number_of_cores = int(os.environ['SLURM_CPUS_PER_TASK'])
        #number_of_cores = math.ceil(cpu_count()/25) # if not on the cluster you should do this instead
        print(number_of_cores*number_of_tasks)
        
        # multiprocssing pool to distribute tasks to:
        print(xfilelst[0])
        with Pool(number_of_cores) as pool:
            # distribute computations and collect results:
            results = pool.map(run_predict, xfilelst)
            
            
            
            
#%%
def run(gpuids, cpuids, i, too_big_predictions, lengthOfExamples):
    batches = []


    with open(os.path.dirname(os.path.realpath("__file__")) + '/test_dataset/data/tooBigPredictions.' + TGT_LANG, "a+", encoding="utf-16") as wTooBig:
...
if __name__ == '__main__':
    ...
    toWrite = []
    data_in = []
    with open(os.path.dirname(os.path.realpath("__file__")) + '/test_dataset/data/test.' + SRC_LANG, "r",
    ...
    if os.path.exists(
            os.path.dirname(os.path.realpath("__file__")) + '/test_dataset/data/predictions.' + TGT_LANG) == False:
        open(os.path.dirname(os.path.realpath("__file__")) + '/test_dataset/data/predictions.' + TGT_LANG, "w").close()

    with open(os.path.dirname(os.path.realpath("__file__")) + '/test_dataset/data/predictions.' + TGT_LANG, "r+",
              encoding="utf-16") as fp:
        i = len(fp.readlines())
    print('Total lines so far:', i)

    if os.path.exists(os.path.dirname(
            os.path.realpath("__file__")) + '/test_dataset/data/tooBigPredictions.' + TGT_LANG) == False:
        open(os.path.dirname(os.path.realpath("__file__")) + '/test_dataset/data/tooBigPredictions.' + TGT_LANG,
             "w").close()

    with open(os.path.dirname(os.path.realpath("__file__")) + '/test_dataset/data/tooBigPredictions.' + TGT_LANG, "r+",
              encoding="utf-16") as fp:
    ...
    run(gpuids, aff, i, too_big_predictions, lengthOfExamples)
