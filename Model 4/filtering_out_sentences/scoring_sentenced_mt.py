...
transformer_test_en_mt = transformer(
...
transformer_test_en_mt.load_weights('/netscratch/abela/transformerbaseline/test_dataset/translator_for_confidence_en_mt')
...
class TranslatorForConfidence(tf.Module):
    ...
    def __call__(self, sentence, sentence_tgt):
        ...
        if self.tgt == "mt":
            ...
            # maltese start token.
            start_end = self.tokenizers.mt.tokenize(sentence_tgt)[0]
        ...
        total_predictions = self.transformer.predict(ds, verbose=0)      
...
translator_for_confidence_mt_en = TranslatorForConfidence(tokenizers, transformer_test_mt_en, "mt") 
translator_for_confidence_en_mt = TranslatorForConfidence(tokenizers, transformer_test_en_mt, "en")    
# -

# %%time
res = translator_for_confidence_mt_en(tf.constant(sentence), tf.constant(ground_truth))
print(res)
...
tokenizer_mt = AutoTokenizer.from_pretrained("MLRS/BERTu", use_fast=True)
model_mt = AutoModelForMaskedLM.from_pretrained("MLRS/BERTu")
# set device to GPU
...
def lm_en_prob(sentence):
    ...
    tokenize_input = tokenizer_en(sentence, truncation=True, max_length=512, padding=True, return_tensors='pt')["input_ids"]

    
    tokenize_input.to(device)
    loss = model_en(tokenize_input)[0]
    total_scores = []
...
def lm_mt_prob(sentence):
    tokenize_input = tokenizer_mt(sentence, truncation=True, max_length=512, padding=True, return_tensors='pt')["input_ids"]

    tokenize_input.to(device)
    loss = model_mt(tokenize_input)[0]
    total_scores = []
...
def mt_en_prob(sentence_en, sentence_mt):
#     return [0]
    return translator_for_confidence_en_mt(tf.constant(sentence_en), tf.constant(sentence_mt))



def mt_mt_prob(sentence_mt, sentence_en):
#     return [0]
    return translator_for_confidence_mt_en(tf.constant(sentence_mt), tf.constant(sentence_en))



def sentence_score(sentence_en, sentence_mt, score):
    lm_en = lm_en_prob(sentence_en)
    lm_mt = lm_mt_prob(sentence_mt)

    for i, sentence in enumerate(sentence_en):
        if score[i] == 0:
            mt_en = mt_en_prob(sentence, sentence_mt[i])
            mt_mt = mt_mt_prob(sentence_mt[i], sentence)
            lm_scores = ((1 / len(lm_en[i])) * sum(lm_en[i])) + (
                    (1 / len(lm_mt[i])) * sum(lm_mt[i]))
            mt_scores = ((1 / len(mt_en)) * sum(mt_en)) + (
                    (1 / len(mt_mt)) * sum(mt_mt))

            # print(lm_scores)
            score[i] = (1 / 4) * (lm_scores.cpu() + mt_scores)
    try:
        del lm_en, lm_mt, mt_en, mt_mt, lm_scores, mt_scores
    except:
        print("Can't delete...")
    return score
#print(torch.cuda.memory_allocated())
...
if __name__ == '__main__':
    ...
    os.system('pip install sentence_splitter')
    os.system('pip install -q tfds-nightly')
    os.system('pip install tensorflow_datasets')
    os.system('pip install sacrebleu')
    os.system('pip install jupyter-resource-usage')
    os.system('pip install tensorflow_datasets')
    os.system('pip install tensorflow_text')
    os.system('pip install ipywidgets')
    os.system('pip install tensorflow-model-optimization')
    os.system('pip install -U "tensorflow-text==2.8.*"')
    import math
    
    import langid
    parser = argparse.ArgumentParser("simple_example")
    parser.add_argument("inp_file", help="Place to read the data from")
    parser.add_argument("out_file", help="Place to write the filtered data")
    ...
    lines_to_remove = []
    with open(args.inp_file + '.en', "r", encoding="utf-8") as f, open(args.out_file + '.en', "w", encoding="utf-8") as en, open(args.out_file + '.mt', "w", encoding="utf-8") as mt:
        print("read1")
        sentences_en = f.read().split('\n')
        with open(args.inp_file + '.mt', "r", encoding="utf-8") as f:
            print("read2")
            sentences_mt = f.read().split('\n')
...
#                torch.cuda.empty_cache() # PyTorch thing
                with torch.no_grad():
                    ...
                    if (i*batch_size)+batch_size >= length_sentences_en:
                        ...
                        sentence_mt = sentences_mt[(i*batch_size):length_sentences_en]
                        scores = np.zeros(length_sentences_en-i*batch_size)
                    else:
                        sentence_en = sentences_en[i*batch_size:(i*batch_size)+batch_size]
                        sentence_mt = sentences_mt[i*batch_size:(i*batch_size)+batch_size]
                        scores = np.zeros(batch_size)

                    for j, sentence in enumerate(sentence_en):
                        response = langid.classify(sentence)
                        if response[0] != "en":
                            print("Not English: " + sentence)
                            scores[j] = -1

                    for j, sentence in enumerate(sentence_mt):
                        response = langid.classify(sentence)
                        if response[0] != "mt":
                            print("Not Maltese: " + sentence)
                            scores[j] = -1

                    copy_of_sent_en = copy.deepcopy(sentence_en)
                    copy_of_sent_mt = copy.deepcopy(sentence_mt)
 
                    scores = sentence_score(sentence_en, sentence_mt, scores)
                    #print(scores)
                    for i, score in enumerate(scores):
                        if score < -7:
                            lines_to_remove.append(i)
                        else:
                            en.write(copy_of_sent_en[i]+"\n")
                            mt.write(copy_of_sent_mt[i]+"\n")
