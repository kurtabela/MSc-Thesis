...
# print(torch.cuda.memory_allocated())
# ic_en = tf.saved_model.load('/netscratch/abela/transformerbaseline/ic_en_dataset/translator_exp_ic_en')
# ic_en(tf.constant("I read about triceratops in the book."))
...
tokenizer_ic = AutoTokenizer.from_pretrained("MLRS/BERTu", use_fast=True)
model_ic = AutoModelForMaskedLM.from_pretrained("MLRS/BERTu")
# set device to GPU
...
def lm_ic_prob(sentence):
    tokenize_input = tokenizer_ic(sentence, truncation=True, max_length=512, padding=True, return_tensors='pt')["input_ids"]

    tokenize_input.to(device)
    loss = model_ic(tokenize_input)[0]
    total_scores = []
...
def sentence_score(sentence_en, sentence_ic, score):
    ...
    for i, sentence in enumerate(sentence_en):
        if score[i] == 0:
            ic_en = ic_en_prob(sentence, sentence_ic[i])
            ic_ic = ic_ic_prob(sentence_ic[i], sentence)
            lm_scores = ((1 / len(lm_en[i])) * sum(lm_en[i])) + (
                    (1 / len(lm_ic[i])) * sum(lm_ic[i]))
            ic_scores = ((1 / len(ic_en)) * sum(ic_en)) + (
                    (1 / len(ic_ic)) * sum(ic_ic))

            # print(lm_scores)
            score[i] = (1 / 4) * (lm_scores.cpu() + ic_scores)
    try:
        del lm_en, lm_ic, ic_en, ic_ic, lm_scores, ic_scores
    except:
        print("Can't delete...")
    return score
#print(torch.cuda.memory_allocated())
...
if __name__ == '__main__':
    ...
    lines_to_remove = []
    with open(args.inp_file + '.en', "r", encoding="utf-8") as f, open(args.out_file + '.en', "w", encoding="utf-8") as en, open(args.out_file + '.ic', "w", encoding="utf-8") as ic:
        print("read1")
        sentences_en = f.read().split('\n')
        with open(args.inp_file + '.ic', "r", encoding="utf-8") as f:
            print("read2")
            sentences_ic = f.read().split('\n')
            length_sentences_en = len(sentences_en)
            # result = Parallel()(delayed(inner_for)(i,sentences_en, sentences_ic, lines_to_remove) for i in range(30 - 1))
...
#                torch.cuda.empty_cache() # PyTorch thing
                with torch.no_grad():
                    ...
                    for j, sentence in enumerate(sentence_ic):
                        response = langid.classify(sentence)
                        if response[0] != "is":
                            print("Not ICELANDIC: " + sentence)
                            scores[j] = -1

                    copy_of_sent_en = copy.deepcopy(sentence_en)
                    copy_of_sent_ic = copy.deepcopy(sentence_ic)
                    ...
                    for i, score in enumerate(scores):
                        if score < -7:
                            ...
                            ic.write(copy_of_sent_ic[i]+"\n")
#                 result = Parallel()(delayed(inner_for)(i,sentences_en, sentences_ic, lines_to_remove, length_sentences_en) for i in range(math.ceil((length_sentences_en - 1) / batch_size)))
    print("FINISHED")
# -
