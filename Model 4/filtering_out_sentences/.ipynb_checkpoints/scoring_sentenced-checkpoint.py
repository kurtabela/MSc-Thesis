...
def lm_mt_prob(sentence):
    ...
    tokenize_input.to(device)
    loss = model_mt(tokenize_input.cuda())[0]
    total_scores = []
...
if __name__ == '__main__':
    ...
    #for path in ["dev"]:
        lines_to_remove = []
        with open('../original_data/data/' + path + '.en', "r", encoding="utf-8") as f:
            print("read1")
            sentences_en = f.read().split('\n')
            with open('../original_data/data/' + path + '.mt', "r", encoding="utf-8") as f:
                print("read2")
    ...
    #                 result = Parallel()(delayed(inner_for)(i,sentences_en, sentences_mt, lines_to_remove, length_sentences_en) for i in range(math.ceil((length_sentences_en - 1) / batch_size)))
        ...
        with open('../test_dataset/data/' + path + '.en', "w", encoding="utf-8") as en, open('../test_dataset/data/' + path + '.mt', "w", encoding="utf-8") as mt:
            for pos, line in enumerate(sentences_en):
                if pos not in lines_to_remove:
                    en.write(line+"\n")
                    mt.write(sentences_mt[pos]+"\n")
                else:
                    print(line)
                    print(sentences_mt[pos])
