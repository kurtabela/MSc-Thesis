...
total_lines=$(wc -l < /netscratch/abela/transformerbaseline/ic_en_dataset/data/test.en )
((lines_per_file = (total_lines ) / (${SLURM_NTASKS}-1)))

split --lines=${lines_per_file} --numeric-suffixes=1 --suffix-length=2 --additional-suffix=".en" --verbose /netscratch/abela/transformerbaseline/ic_en_dataset/data/test.en /netscratch/abela/transformerbaseline/ic_en_dataset/data/test



total_lines=$(wc -l < /netscratch/abela/transformerbaseline/ic_en_dataset/data/test.ic )
((lines_per_file = (total_lines) / (${SLURM_NTASKS}-1)))

split --lines=${lines_per_file} --numeric-suffixes=1 --suffix-length=2 --additional-suffix=".ic" --verbose /netscratch/abela/transformerbaseline/ic_en_dataset/data/test.ic /netscratch/abela/transformerbaseline/ic_en_dataset/data/test
