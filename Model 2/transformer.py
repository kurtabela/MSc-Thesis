...
get_ipython().system('pip install -U "tensorflow-text==2.6.*"')
...
# %%
# %%time
res = translator([tf.constant(ground_truth), tf.constant(sentence)])
print(res)


# %%
class TranslatorTestAttention(tf.Module):
  def __init__(self, tokenizers, transformer_test):
    self.tokenizers = tokenizers
    self.transformer_test = transformer_test

  def __call__(self, sentence, max_length=80):
...
#     sentence = self.tokenizers.pt.tokenize(sentence).to_tensor()
    ...
    for i in tf.range(max_length):
      ...
      predictions, _ = self.transformer_test([encoder_input, output], training=False)
...
translator = TranslatorTestAttention(tokenizers, transformer_test)
