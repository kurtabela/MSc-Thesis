...
interpreter = tf.lite.Interpreter(model_path='test_dataset/quantized_and_pruned_' + SRC_LANG + "_" + TGT_LANG + '.tflite')
...
class Translator(tf.Module):
    ...
    def __call__(self, sentences, beam_width=5):
        ...
        if SRC_LANG == "en" and TGT_LANG == "mt":
            ...
            vocab_length = self.tokenizers.mt.get_vocab_size()
        else:
        ...
        for i, score in enumerate(scores):
            ...
            if SRC_LANG == "en" and TGT_LANG == "mt":
                text = self.tokenizers.mt.detokenize(output)
...
#             print(np.shape(batch_inputs_enc))
#             print(np.shape(batch_inputs_dec))
#             print(encoder_input[0])
#             print([alive_seq[0]])
#             print(tf.pad(
#                     [alive_seq[0][0]], [[0, 0, ], [0, MAX_TOKEN_LENGTH - tf.shape([alive_seq[0][0]])[1]]], "CONSTANT"))
            
            flat_logits = []
            for i, inp in enumerate(encoder_input_reshaped):
                flat_logits.append(predict(inputs=np.float32(encoder_input_reshaped[i]), dec_inputs=np.float32(tf.pad(
                    [alive_seq[0][i]], [[0, 0, ], [0, MAX_TOKEN_LENGTH - tf.shape([alive_seq[0][i]])[1]]], "CONSTANT")))["outputs"][:, -1:, :])
#             print(flat_logits)
#             print(flat_logits[0])
...
#                 print(len(flat_logits[i][0][0]))
#                 print(padding_applied)
#                 print(np.delete(flat_logits[i], np.s_[-padding_applied[i]:], 0))
#                 new_array.append(np.delete(flat_logits[i], np.s_[-padding_applied[i]:], 0)[0])
...
# run(10, i, too_big_predictions, lengthOfExamples)
...
#translator = Translator(tokenizers, transformer_test, SRC_LANG, TGT_LANG, tokenize_pairs, beam_search)  
...
#
#
#    def start(self, xfilelst, nValue):
#    
#        with open(os.path.dirname(os.path.realpath("__file__")) + '/test_dataset/data/predictions' + str(nValue) + '.' + TGT_LANG, "a+",
#          encoding="utf-8") as w:
#    
#            for xfile in xfilelst:
#                print("PRINTING TO " + os.path.dirname(os.path.realpath("__file__")) + '/test_dataset/data/predictions' + str(nValue) + '.' + TGT_LANG)
#                output = translator(xfile)
#                #print("predicted")
...
# %%
def run(task_id, i, too_big_predictions, lengthOfExamples, test_tgt_lang):
    batches = []

    with open(os.path.dirname(os.path.realpath("__file__")) + '/test_dataset/data/predictions' + str(task_id) + '.' + TGT_LANG, "a+",
              encoding="utf-8") as w:
        with open(os.path.dirname(os.path.realpath("__file__")) + '/test_dataset/data/actualTest' + str(task_id) + '.' + TGT_LANG, "a+", encoding="utf-8") as wTooBig:
            print("doing while....")
            print(i+too_big_predictions + batch_size)
            ...
            while (i+too_big_predictions + batch_size) <= lengthOfExamples-1:
                ...
                to_predict = []
                if SRC_LANG == "en":
                    for curr_index,en in enumerate(en_list[i+too_big_predictions: i +too_big_predictions + batch_size]):
                        try:
                            ...
                            print("some error with: " + str(i+too_big_predictions+curr_index))
                            too_big_predictions += 1

                if SRC_LANG == "mt":
                    for curr_index, mt in enumerate(mt_list[i+too_big_predictions: i +too_big_predictions + batch_size]):
                ...
                if to_predict:
                   ...
                   wTooBig.write(test_tgt_lang[i+too_big_predictions+curr_index].replace("\n", ""))
                   wTooBig.write("\n")
                   #batches.append(to_predict)
                   print("PRINTING TO " + os.path.dirname(os.path.realpath("__file__")) + '/test_dataset/data/predictions' + str(task_id) + '.' + TGT_LANG)
                   output = translator(to_predict)
                   #print("predicted")
                   res = [el[0].numpy().decode('UTF-8') for el in output]
                   w.write("\n".join(res))
                   w.write("\n")
                   print("GOT: " + str(data_in[i+too_big_predictions+curr_index].replace("\n", "")) + "\nAnd Received: " + "\n".join(res) + "\n\n")
                   #print("worker predicted ")
                # batches.append(to_predict)
...
if __name__ == '__main__':
    ...
    toWrite = []
    data_in = []
    with open(os.path.dirname(os.path.realpath("__file__")) + '/test_dataset/data/test' + str(args.task_id) + '.' + SRC_LANG, "r+",
              encoding="utf-8") as f:
        data_in = f.read().split('\n')
    with open(os.path.dirname(os.path.realpath("__file__")) + '/test_dataset/data/test' + str(args.task_id) + '.' + TGT_LANG, "r+",
              encoding="utf-8") as f:
        test_tgt_lang = f.read().split('\n')
    ...
    open(os.path.dirname(os.path.realpath("__file__")) + '/test_dataset/data/predictions' + args.task_id + '.' + TGT_LANG, "w+").close()
    open(os.path.dirname(os.path.realpath("__file__")) + '/test_dataset/data/actualTest' + str(
            args.task_id) + '.' + TGT_LANG, "w+").close()

    with open(os.path.dirname(os.path.realpath("__file__")) + '/test_dataset/data/predictions'+ args.task_id + '.' + TGT_LANG, "r+",
              encoding="utf-8") as fp:
        i = len(fp.readlines())
    print('Total lines so far:', i)

    if os.path.exists(os.path.dirname(
            os.path.realpath("__file__")) + '/test_dataset/data/actualTest' + args.task_id + '.' + TGT_LANG) == False:
        open(os.path.dirname(os.path.realpath("__file__")) + '/test_dataset/data/actualTest' + args.task_id + '.' + TGT_LANG,
             "w+").close()

    with open(os.path.dirname(os.path.realpath("__file__")) + '/test_dataset/data/actualTest' + args.task_id + '.' + TGT_LANG, "r+",
              encoding="utf-8") as fp:
    ...
    print(i)
    #data_in = data_in[:i + too_big_predictions + 1000]
    lengthOfExamples = len(data_in)
    print(lengthOfExamples)

    run(args.task_id, i, too_big_predictions, lengthOfExamples, test_tgt_lang)
